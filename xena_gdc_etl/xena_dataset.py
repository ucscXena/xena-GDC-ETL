...
class TCGAPhenoset(XenaDataset):
    ...
    def __process_one_biospecimen_supplement(self, path):
        """Extract info from GDC's biospecimen supplement and re-organize them
        into a pandas DataFrame.
        ...
        if ext != '.xml':
            raise IOError(
                'Unknown file type for TCGA biospecimen data: {}'.format(ext)
            )
    ...
    def transform(self):
        ...
        message = 'Make Xena matrix for raw phenotype data of {}.'
        ...
        for path in self.raw_data_list:
            ...
            # `read_biospecimen` and `read_clinical` will check file format
            try:
                df = self.__process_one_clinical_supplement(path)
                if not df.empty:
                    clin_dfs.append(df)
            except Exception:
                try:
                    df = self.__process_one_biospecimen_supplement(path)
                    if not df.empty:
                        bio_dfs.append(df)
                except Exception:
                    raise TypeError('Fail to process file {}.'.format(path))
