  ...
  **Table of Contents**
  ...
  - Clone the repository: ``git clone https://github.com/yunhailuo/xena-GDC-ETL.git``.
  - To install the dependencies in a virtual environment, first create the environment as:
    > ``sudo apt-get install python3-venv``
      ``python3 -m venv gdc-env``,

      where `gdc-env` is just a name, and can be anything you like.
  - Activate the virtual environment as:
    > ``source gdc-env/bin/activate``

  - Now, `cd xena-GDC-ETL` to the get in the project directory.
  - To install dependencies in the activated environment, run
    > ``pip install -r requirements.txt``

    This will install all requirements, which will only be usable when the environment
    is activated.


  ### For Windows

  - Make sure you have Git Bash and Python3 installed.
  - Clone using the repository: ``git clone https://github.com/yunhailuo/xena-GDC-ETL.git``
  - If Python3 is installed, run `python -m venv gdc-env` to create the environment.
  - Run `gdc-env\Scripts\bin\activate` to activate it. Again, `gdc-env` can be any name you like.
  - `cd xena-GDC-ETL` to the get in the project directory.
  -  Now, ``pip install -r requirements.txt`` should install the requirements for the project.
