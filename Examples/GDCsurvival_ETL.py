...
"""A example script for importing survival data from GDC API to Xena
...
def main():
    ...
    for project in projects:
        counts += 1
        msg = '[{}/{}] Importing survival data for projects: {}'
        print(msg.format(counts, total_projects, project))
        try:
            survivalset = GDCSurvivalset(project, root_dir)
            survivalset.download().transform().metadata()
        except Exception:
